...
bool WLDMaterialPalette::explodeName(MaterialDefFragment *def, QString &charName,
                                     uint32_t &skinID, QString &partName)
{
    return explodeName(def->name(), charName, skinID, partName);
}
...
QString WLDMaterialPalette::materialName(QString defName)
{
    ...
    uint32_t skinID = 0;
    if(explodeName(defName, charName, skinID, partName))
...
uint32_t WLDMaterialPalette::findMaterialIndex(WLDMaterialSlot *slot, uint32_t skinID) const
{
    if(!slot)
        return -1;
...
void WLDMaterialPalette::makeSkinMap(uint32_t skinID, MaterialMap *materialMap) const
{
    ...
    {
        WLDMaterialSlot *slot = m_materialSlots[i];
        materialMap->setMappingAt(i, findMaterialIndex(slot, skinID));
    }
}
...
WLDMaterialSlot::WLDMaterialSlot(QString matName)
{
    QString charName;
    uint32_t skinID = 0;
    if(!WLDMaterialPalette::explodeName(matName, charName, skinID, slotName))
